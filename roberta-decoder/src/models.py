...
class RobertaEncoder(nn.Module):
    def __init__(self, roberta_model_name='roberta-base', hidden_size=512, dropout=0.5):
        super(RobertaEncoder, self).__init__()
        self.hidden_size = hidden_size
        self.roberta = RobertaModel.from_pretrained(roberta_model_name)
        self.roberta_hidden_size = self.roberta.config.hidden_size  # 768 for roberta-base
        
        # Project RoBERTa output to desired hidden size
        self.projection = nn.Linear(self.roberta_hidden_size, hidden_size)
        self.dropout = nn.Dropout(dropout)

    def forward(self, input_ids, input_lengths):
        """
        Args:
            input_ids: (seq_len, batch_size) - RoBERTa token IDs
            input_lengths: list of actual sequence lengths
        Returns:
            encoder_outputs: (seq_len, batch_size, hidden_size) - encoded sequence
            encoder_hidden: (batch_size, hidden_size) - pooled representation for root
        """
        # Transpose to (batch_size, seq_len) for RoBERTa
        ...
        last_hidden_states = outputs.last_hidden_state  # (batch_size, seq_len, roberta_hidden_size)
        pooler_output = outputs.pooler_output  # (batch_size, roberta_hidden_size)
        
        # Project to desired hidden size
        projected_sequence = self.projection(last_hidden_states)  # (batch_size, seq_len, hidden_size)
        projected_pooled = self.projection(pooler_output)  # (batch_size, hidden_size)
        
        # Apply dropout
        projected_sequence = self.dropout(projected_sequence)
        projected_pooled = self.dropout(projected_pooled)
        
        # Transpose back to (seq_len, batch_size, hidden_size) for compatibility
        encoder_outputs = projected_sequence.transpose(0, 1)
        
        return encoder_outputs, projected_pooled
